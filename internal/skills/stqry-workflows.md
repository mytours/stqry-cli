...
- **Per-stop screen composition** — one `story` screen per stop with the image first, then audio, then text sections. Audio sits right under the image so the user can tap play without scrolling:
  ...
  3. `text` section
  - **Variation: cover image as screen header instead of a section.** Set `--header-layout image_and_title` (or `image`, or `tall`) on the screen — the `cover_image_media_item_id` you already set for list / grid / wide layouts will also render as the screen header, and the first `single_media` section can be dropped. Results in audio + text only on the stop screen. This is the preferred shape when the header image and the section image would be identical (the usual case for a tour stop).
