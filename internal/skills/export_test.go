...
func TestBuildZip_DifferentVersionsProduceDifferentContent(t *testing.T) {
	d1, _ := skills.BuildZip("v1.0.0")
	d2, _ := skills.BuildZip("v2.0.0")

	r1, _ := zip.NewReader(bytes.NewReader(d1), int64(len(d1)))
	r2, _ := zip.NewReader(bytes.NewReader(d2), int64(len(d2)))
