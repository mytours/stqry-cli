...
- `--thumbnail-media-item-id` flag on `stqry media create` and `stqry media update` for setting the `thumbnail_media_item_id` field (the poster image the player shows behind an audio or video item). Previously there was no way to set this field from the CLI, forcing callers to drop out to curl for what is otherwise a routine part of building an audio tour (each stop's audio MediaItem wants the stop's image as its thumbnail so the player surface doesn't render a blank tile). `--thumbnail-media-item-id=0` on update clears the field.
- `--header-layout` flag on `stqry screens create` and `stqry screens update`, accepting the `ScreenPartial.header_layout` enum (`none`, `image`, `image_and_title`, `short`, `tall`) with client-side validation. Addresses a request from Lydia: "change section single image to tour stop header layout instead". Each stop screen already carries its image via `cover_image_media_item_id`; flipping `header_layout` to `image` / `image_and_title` / `tall` promotes that image into the actual header, so the redundant `single_media` section that used to sit at the top of each stop (per Workflow 5) can be deleted. `stqry-reference` gets a new "Screen header layouts" section with a retrofit shell loop that walks a tour, strips the first `single_media` section from each stop, and sets `--header-layout image_and_title`. Workflow 5 gains a Variation note pointing at the same approach as the preferred shape when the header image and the top-section image would be identical.
- Full link-item surface on `stqry screens sections links add` / `update`: `--link-type` (23-value enum, validated client-side), `--link-text`, `--url`, `--username`, `--icon-type` (`media_item` / `stock_icon` / `clear`), `--item-type` (`Bundle` / `Collection` / `CollectionItem` / `Screen` / `MediaItem` / `CrossRegionLink`), `--item-id`, `--position`. Also on `social` sub-items: `--social-network` (12-value enum), `--username`, `--link-text`, `--position`. All flag names are kebab-case; the PATCH body uses the correct snake_case API field names (`link_type`, `link_text`, `item_type`, `item_id`, `social_network`). TranslatedString fields (`link_text`, `url`, `username`) are wrapped in a `{<lang>: value}` map, keyed by `--lang` (default `en`). Pre-refactor the sub-item `add` / `update` commands were using a raw Visit() passthrough, so `--link-type` reached the API as `{"link-type": "..."}` and `--label` as `{"label": "..."}` (the real field is `link_text`) — both silently ignored by the server. Covers Lydia's "be able to set link types for individual links in a link group, including social links". The icon for a link is auto-chosen from its `link_type` (globe for `url`, the network's logo for social types, etc.); Builder doesn't let users pick among stock icons, so the CLI doesn't pretend to either — an earlier draft shipped a `--stock-icon` flag that let users pass arbitrary names; it was walked back after Ilia confirmed the Builder behaviour. `--icon-type clear` still hides the icon; `media_item` is an internal Builder mode and not configurable from this CLI.
- `--layout` flag on `stqry screens sections update`, validated against the union of `link_group` (10 values incl. `button`, `button_with_icon`, `list`, `icon`, `grid_image`, `wide_image`, `horizontal_slider`) and `social_group` (`icons`, `list`) layouts. Addresses "change all links to buttons" — the section's `layout` is the knob that controls whether its link_items render as list rows, buttons, or something else. Bulk pattern (per the now-familiar composition style): `for sid in $(stqry collections items list 42 --jq '.[].item_id'); do for sec in $(stqry screens sections list $sid --jq 'map(select(.type=="link_group"))[].id'); do stqry screens sections update $sec --screen-id $sid --layout button_with_icon; done; done`. Added the snippet to the stqry-reference skill under a new "link_group layout" section.
